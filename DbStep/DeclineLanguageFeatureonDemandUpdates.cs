...
namespace WSUSMaintenance.DbStep
{
    ...
    public class DeclineLanguageFeatureonDemandUpdates : IStep
    {
        ...
        public Result Run()
        {
            ...
            try
            {
                WriteLine("Decline Language Feature on Demand Updates");
                using (var dbconnection = new SqlConnection(wsusConfig.Database.ConnectionString))
                {
                    dbconnection.InfoMessage += (sender, e) =>
                    {
                        WriteLine($"DeclineLanguageFeatureonDemandUpdates - TSQL - {e.Source}-{e.Message}");
                    };
