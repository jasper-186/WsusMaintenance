...
{
    public class CleanupUnneededContentFiles : IStep
    {
        ...
        public Result Run()
        {
            ...
            try
            {
                WriteLine("Cleanup Unneeded Content Files");
                using (var dbconnection = new SqlConnection(wsusConfig.Database.ConnectionString))
                {
                    dbconnection.InfoMessage += (sender, e) =>
                    {
                        WriteLine($"CleanupUnneededContentFiles - TSQL - {e.Source}-{e.Message}");
                    };
                    dbconnection.Open();
                    var cmd = dbconnection.CreateCommand();
                    cmd.CommandText = "EXEC spCleanupUnneededContentFiles";
