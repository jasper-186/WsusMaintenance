...
{
    public class SetManualSyncronizationSchedule : IStep
    {
        ...
        public Result Run()
        {
            ...
            try
            {
                WriteLine("Set Manual Syncronization Schedule");
                using (var dbconnection = new SqlConnection(wsusConfig.Database.ConnectionString))
                {
                    dbconnection.InfoMessage += (sender, e) =>
                    {
                        WriteLine($"SetManualSyncronizationSchedule - TSQL - {e.Source}-{e.Message}");
                    };
                    dbconnection.Open();

                    var cmd = dbconnection.CreateCommand();
                    cmd.CommandText = "UPDATE [dbo].[tbConfiguration] SET [Value] = 0 WHERE [Name] = 'NameSubscriptionSynchronizeAutomatically'";
        ...
        {
            if (!wsusConfig.Steps.DatabaseSteps["SetManualSyncronizationSchedule"])
            {
                return false;
            }
